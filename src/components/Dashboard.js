...
class Dashboard extends Component {
  ...
  _getDailyPercent(macro, date, forDailyGoal = false) {
    const mealsThisDate = this.props.meals.filter(meal =>
      meal.createdAt.includes(moment(date).format("YYYY-MM-DD"))
    );
    const macrosConsumedCollection = mealsThisDate.map(meal =>
      parseInt(meal.macros()[macro] || 0)
    );
    const totalMacrosThisDate =
      macrosConsumedCollection.length === 0
        ? 1
        : macrosConsumedCollection.reduce(
            (macrosTotal, macros) => macrosTotal + macros
          );
    if (forDailyGoal) return totalMacrosThisDate;
    switch (macro) {
      case "calorie":
        return (
          Math.round(totalMacrosThisDate) /
      ...
      case "fat":
        return (
          Math.round(totalMacrosThisDate) / this.props.user.getFatRequirement()
        );
      case "carbohydrate":
        return (
          Math.round(totalMacrosThisDate) /
            ...
            >
              <View style={{ flexDirection: "column", margin: 10 }}>
                ...
                <Progress.Circle
                  ...
                  borderWidth={1}
                  color={"#696e68"}
                  progress={this.state.proteinPercent || 0}
                  size={80}
                  animated={true}
                  indeterminate={false}
                  style={{ top: 10 }}
              ...
              <View style={{ top: 20 }}>
                <Progress.Pie
                  ...
                  color={"#696e68"}
                  showsText={true}
                  progress={this.state.dailyGoalPercent || 0}
                  size={170}
                  animated={true}
                  indeterminate={false}
                  style={{ height: "79%" }}
              ...
              <View style={{ flexDirection: "column", margin: 10 }}>
                <Progress.Circle
                  ...
                  borderWidth={1}
                  color={"#696e68"}
                  progress={this.state.fatPercent || 0}
                  size={80}
                  animated={true}
                  indeterminate={false}
                />
                ...
                <Progress.Circle
                  ...
                  borderWidth={1}
                  color={"#696e68"}
                  progress={this.state.carbohydratePercent || 0}
                  size={80}
                  animated={true}
                  indeterminate={false}
                  style={{ top: 10 }}
    ...
    if (day) {
      return (
        <View style={{ width: "20%", alignItems: "center" }}>
          <Text style={{ fontWeight: "100", fontSize: 28 }}>
            {moment(day.dateString).format("DD")}
          </Text>
          <Text style={{ fontWeight: "100", fontSize: 24 }}>
            {moment(day.dateString).format("ddd")}
          </Text>
        </View>
      );
    } else {
  ...
  _renderCalendarItem = (meal, firstItemInDay) => {
    return (
      <ListDashboard
        key={meal._id}
        meal={meal}
        changeDisplayingMeal={this.props.changeDisplayingMeal}
      />
    );
  };
  ...
  prepareCalendar = () => {
    ...
    this.props.meals.forEach((meal, idx, mealsCollection) => {
      ...
      if (!markedDates[mealDate]) {
        ...
        datesAndMeals[mealDate] = mealsCollection.filter(
          mealInFilter =>
            moment(mealInFilter.createdAt).format("YYYY-MM-DD") === mealDate
        );
      }
    });
    // if no meals today
    ...
    return (
      <LinearGradient colors={["#fff", "#fff", "#fff"]}>
        <Header>
          ...
          <Body>
            <Title style={{ fontWeight: "300", bottom: 5, fontSize: 24 }}>
          ...
          <Right>
            <TouchableOpacity onPress={this.openMenu}>
              <Image
                ...
                style={{ height: 35, width: 35, margin: 10, bottom: 8 }}
              />
            </TouchableOpacity>
          </Right>
        </Header>
        ...
        <View style={{ height: screen.height / 2, padding: 5 }}>
          <Agenda
            ...
            renderItem={(item, firstItemInDay) => {
              return this._renderCalendarItem(item, firstItemInDay);
            }}
            // specify how each date should be rendered. day can be undefined if the item is not first in that day.
            renderDay={(day, item) => {
              return this._renderCalendarDay(day, item);
            }}
            // specify how empty date content with no items should be rendered
            renderEmptyDate={date => {
              return this._renderEmptyItem(date);
            }}
